    ...
    if (p == -1) {
        ...
        if (WIFEXITED(codeRetour)) {
            ...
            printf("Terminaison normale de P3 avec erreur (code de sortie est : %d)\n", statut);
                        ...
                        } else if(WIFSTOPPED(codeRetour)){//Si le processus p3 a ete arrete
                        int signal = WSTOPSIG(codeRetour);
                        printf("Terminaison anormale de p3 : arret (signal %d):\n",signal);
                        }
                        else if (WIFSIGNALED(codeRetour)){//Si le processus p3 a ete tue
                        int signal = WTERMSIG(codeRetour);
                        printf("Terminaison anormale de p3 : signal (signal %d):\n",signal);
                        }
